...
public class ExtendedGuiChest extends AbstractContainerScreen
{
    ...
    protected void drawBackground(float partialTicks, int mouseX, int mouseY)
    {
        ...
        if (separateBlits) {
            this.blit(left, top, 0, 0, this.containerWidth, this.inventoryRows * 18 + 17);
            this.blit(left, top + this.inventoryRows * 18 + 17, 0, 126, this.containerWidth, 96);
        } else {
        ...
        for (int i=0; i<9; i++) {
            this.drawTexturedModalRectWithMouseHighlight(left+7+i*18,    top+-18,                          1*18, 2*18, 18, 18, mouseX, mouseY);       // arrow down above chests
            this.drawTexturedModalRectWithMouseHighlight(left+7+i*18,    top+40+(this.inventoryRows+4)*18, 9*18, 2*18, 18, 18, mouseX, mouseY);       // arrow up below player inv
        }
        ...
        for (int i=0; i<rowsToDrawDownArrow; i++) {
            this.drawTexturedModalRectWithMouseHighlight(left+ -18,      top+17+i*18,                      1*18, 2*18, 18, 18, mouseX, mouseY);       // arrow down left of chest
        }
        for (int i=0; i<4; i++) {
            this.drawTexturedModalRectWithMouseHighlight(left+ -18,      top+28+(i+this.inventoryRows)*18, 9*18, 2*18, 18, 18, mouseX, mouseY);       // arrow up left of player inv
        }

        this.drawTexturedModalRectWithMouseHighlight(left+this.containerWidth, top+17,                           
                11*18, 0*18, 18, 18, mouseX, mouseY);       // broom chest
        this.drawTexturedModalRectWithMouseHighlight(left+this.containerWidth, top+17+18,
        ...
        for (int i=0; i<36; i++) {
            if (!hasShiftDown() && FrozenSlotDatabase.isSlotFrozen(i)) {
                ...
                this.blit(left+slot.xPosition, top+slot.yPosition, 7*18+1, 3*18+1, 16, 16);               // stop sign
            }
        }
        
        myTooltip(left+this.containerWidth, top+17,  18, 18, mouseX, mouseY, I18n.translate("easierchests.sortchest"));
        myTooltip(left+this.containerWidth, top+17+18, 18, 18, mouseX, mouseY, I18n.translate("easierchests.matchdown"));
    }
    ...
    void checkForMyButtons(double mouseX, double mouseY) {
        if (mouseX>=left-18 && mouseX<=left) {                                        // left buttons
            int deltay = (int)mouseY-top;
        ...
        } else if (mouseX>left+this.containerWidth && mouseX <= left+this.containerWidth+18) {   // right buttons
            if (mouseY>top+17 && mouseY<top+17+18)
                sortInventory(true);
            else if (mouseY > top+17+18 && mouseY < top+17+36)
                moveMatchingItems(true);
            /* else if (mouseY>top+28+(this.inventoryRows)*18 && mouseY<top+28+(this.inventoryRows)*18+18)
                sortInventory(false);
            else if (mouseY>top+28+(this.inventoryRows)*18+18 && mouseY<top+28+(this.inventoryRows)*18+36)
                moveMatchingItems(false);                       */
        } else if (mouseX>left+7 && mouseX<left+7+9*18) {                             // top/bottom buttons
            boolean isChest;
            if (mouseY>top-18 && mouseY<top)
                isChest=true;
            else if (mouseY>top+40+(this.inventoryRows+4)*18 && mouseY<top+40+(this.inventoryRows+4)*18+18)
                isChest=false;
            else
                return;
            int column=((int)mouseX-left-7)/18;
    ...
    public static void moveMatchingItems(AbstractContainerScreen screen, boolean isChestToPlayer) {
        ...
        for (int i=0; i<fromSize; i++) {
            ...
            for (int j=0; j<toSize; j++) {
                ...
                if (fromStack.isItemEqualIgnoreDamage(toStack)
